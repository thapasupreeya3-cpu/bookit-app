'use strict';
/* Shared booking diary with date-based carer selection. */
window.CareBookingCalendar = (() => {
  ...
  const canBook = () => !!API.me && !API.me.admin && (API.me.role==='participant' || (API.me.role==='coordinator' && API.actingFor?.id && (!Array.isArray(API.actingFor.scopes) || API.actingFor.scopes.includes('bookings'))));
  function setup(){
  ...
  function bind(s){
    s.wrap.onclick=event=>{const b=event.target.closest('[data-cal-action]');if(!b||!current(s))return;event.preventDefault();const action=b.dataset.calAction;
      if(action==='day'){s.selected=b.dataset.date;paint(s,true);if(canBook()&&s.selected>=s.data.today)window.CareBookingCarerPicker?.open({date:s.selected,returnFocus:s.wrap.querySelector(`[data-date="${s.selected}"]`)});}
      else if(action==='book'&&canBook()&&s.selected>=s.data.today)window.CareBookingCarerPicker?.open({date:s.selected,returnFocus:b});
