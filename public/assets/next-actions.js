...
window.CareNextActions = (() => {
  ...
  function action(task, role, destination) {
    ...
    return {group:href,title:task.label || 'Review this task',hint:task.detail || '',verb,href};
  }
  ...
  function render(data,destination=t=>t.destination) {
    ...
    const heading=mine.length?mine.length+' action'+(mine.length===1?'':'s')+' need'+(mine.length===1?'s':'')+' your attention':office.length?'Nothing you need to do right now':'You are up to date';
    const hint=mine.length?'Start with the first item. Related steps are grouped together.':office.length?'Your outstanding reviews are with the office.':'New requests and anything due will appear here.';
