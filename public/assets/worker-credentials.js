...
window.WorkerCredentials = (() => {
  ...
  function page(s) {
    const reqs = requirements(s.types,s.docs).sort((a,b)=>Number(a.have)-Number(b.have)||Number(a.verified)-Number(b.verified)), missing = reqs.filter(r=>!r.have), pending=reqs.filter(r=>r.have&&!r.verified), verified=reqs.filter(r=>r.verified);
    ...
    return `<section class="credentials-workspace" aria-label="Credentials and checks"><h2>Credentials &amp; checks</h2><p>Add or replace your files here. The office records verification after reviewing them.</p>
      ...
      <section class="cred-upload" aria-labelledby="credUploadTitle"><h3 id="credUploadTitle">Add a document</h3><label for="credTypeSearch">Find a document type</label><input id="credTypeSearch" type="search" placeholder="Search identity, screening, certificates…" autocomplete="off"><p id="credTypeCount" role="status"></p><label for="credType">Document type — full list</label><select id="credType">${options(s.types,s.categories)}</select><div id="credFormArea"><p class="cred-hint">Select the exact type first so your file is recorded in the right place.</p></div></section>
  ...
  function bind(s) {
    ...
    s.wrap.oninput=event=>{if(event.target.id==='credTypeSearch'&&!s.saving){const query=event.target.value.trim().toLowerCase(),select=s.wrap.querySelector('#credType'),selected=select.value;const list=s.types.filter(t=>t.key===selected||(t.label+' '+t.key+' '+t.category).toLowerCase().includes(query));select.innerHTML=options(list,s.categories);select.value=selected;s.wrap.querySelector('#credTypeCount').textContent=list.length+' document types shown. Clear the search for the full list.';}};
