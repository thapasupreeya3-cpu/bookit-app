...
window.CarePayments = (() => {
  ...
  function state(d){
    ...
    if(d.query_state==='awaiting-participant'||(d.review_state==='pending'&&d.status==='under-review'))return {tone:'waiting',label:'Waiting on participant — payment paused',icon:'◷'};
    if(d.review_state==='queried'||d.status==='under-review')return d.can_manage_query?{tone:'action',label:'Query needs a reply — payment paused',icon:'→'}:{tone:'waiting',label:'Query open — payment paused',icon:'◷'};
  ...
  function controls(d,publicView=false){
    ...
    const funding=d.funding||d.plan,managed=funding==='ndia'||(funding==='plan'&&!publicView),payAfterReview=d.can_pay_after_review===true,reviewAllowed=!publicView&&d.can_review&&!d.can_manage_query,canReview=reviewAllowed&&(d.review_state!=='approved'||d.status==='under-review'),pending=d.payment_pending||d.status==='processing'||d.payment_state==='processing';
    ...
    const queried=d.review_state==='queried',paused=queried||d.status==='under-review',hasReply=d.query_case?.state==='awaiting-participant';
    if(paused)out+='<p class="cp-notice" data-tone="waiting">'+(publicView?'Payment is paused while the authorised participant and office review this shift.':d.can_manage_query?hasReply?'Your reply has been sent for participant review. Payment stays paused until they approve.':'Review the question and send an answer, or ask the worker to respond. Payment stays paused until the participant approves.':hasReply?'The office has replied. Read the response above, then approve if your question is resolved.':'Your question is open. Read the response history above. You can approve once your question is resolved.')+'</p>';
    if(canReview){
      out+=`<form data-cp-form="review" class="cp-review"><h2>${paused?'Is your question resolved?':'Check your completed shift'}</h2><p>Review the dates, hours and charges below before you approve.</p><label class="cp-confirm"><input type="checkbox" name="confirm" required> ${paused?'My question has been resolved, and I have checked the completed shift and its charges.':'I have checked the completed shift and its charges.'}</label><div class="cp-buttons">${!managed&&payAfterReview&&d.checkout_enabled&&methods(d).length?'<button type="submit" class="cp-primary" name="decision" value="pay">Approve & pay</button>':''}<button type="submit" name="decision" value="later">${managed||!payAfterReview?'Approve shift':'Approve — pay by due date'}</button></div><p class="cp-small">Approving confirms the shift. Payment and its due date are shown separately.</p></form>`;
    }else if(d.review_state!=='approved'&&!d.can_manage_query){out+='<p>The authorised participant or helper needs to review this shift before payment is available.</p>';if(publicView&&d.review_url){try{const review=new URL(d.review_url,location.origin);if(review.origin===location.origin&&/^#\/invoice\?/.test(review.hash))out+='<p><a class="cp-link" href="'+e(review.href)+'">Sign in to review this shift</a></p>';}catch{}}}
    if(reviewAllowed)out+=`<details class="cp-query"><summary>${paused?'Send another question':'Query this shift'}</summary><form data-cp-form="query"><label>Your question<textarea name="query_note" required minlength="10" maxlength="4000" rows="3"></textarea></label><button type="submit">Send question</button></form></details>`;
    ...
    if(!pending&&d.status!=='under-review'&&d.review_state==='approved'&&!managed&&d.can_pay&&(!d.checkout_enabled||!methods(d).length))out+='<p class="cp-notice" data-tone="waiting">Online payment is temporarily unavailable. Your shift remains approved. Use Refresh payment status to try again, or contact the office using the details on your invoice.</p>';
    return out;
  }
  function queryPanel(d,publicView=false,draft=null){
    const c=d.query_case,office=!publicView&&d.can_manage_query===true&&!!api()?.me?.admin;
    if(publicView||!c||c.has_history===false||(!office&&!d.can_review))return '';
    const history=Array.isArray(c.history)?c.history:[],bookings=Array.isArray(c.bookings)?c.bookings:[],multiple=bookings.filter(b=>b.approval_state==='queried').length>1,labels={question:'Question', 'worker-reply':'Worker response','office-reply':'Office response',note:'Update'},open=!c.withdrawn&&c.state!=='resolved';
    const notes=history.length?'<ol class="cp-query-history">'+history.map(x=>'<li><p class="cp-query-meta"><strong>'+e(labels[x.kind]||'Update')+'</strong>'+ (x.author?' · '+e(x.author):'')+' · '+e(date(x.created))+'</p><p class="cp-note">'+e(x.body)+'</p></li>').join('')+'</ol>':(c.bookings||[]).filter(b=>b.query_note).map(b=>'<blockquote class="cp-note">'+e(b.query_note)+'</blockquote>').join('')||'<p>No query messages have been recorded.</p>';
    let actions='';
    if(office&&open){
      if(c.can_reply)actions+=`<form data-cp-form="office-reply" data-request-key="${e(draft?.key||requestKey())}" class="cp-query-reply"><label>${multiple?'Answer for the queried shifts':'Answer for the participant'}<textarea name="reply" required minlength="10" maxlength="4000" rows="4">${e(draft?.reply||'')}</textarea></label><label class="cp-confirm"><input type="checkbox" name="confirm" required> I have checked this answer and want the participant to review the invoice again.</label><button type="submit" class="cp-primary">Send reply & request review</button><p class="cp-small">${multiple?'This reply applies to all queried shifts on this invoice. ':''}The participant receives a notification. They decide whether to approve; sending a reply does not resume payment.</p></form>`;
      if(c.worker_requested_at)actions+='<p class="cp-notice">◷ Worker response requested on '+e(date(c.worker_requested_at))+'. Their answer will appear in this history.</p>';
      else if(c.can_request_worker)actions+='<form data-cp-form="request-worker" class="cp-query-worker"><label class="cp-confirm"><input type="checkbox" name="confirm" required> Ask the assigned worker to check the shift and respond.</label><button type="submit">Request worker response</button></form>';
      actions+='<details class="cp-query-correction"><summary>The hours or charge need correcting</summary><p>'+(c.can_withdraw===false?'Payment has already been recorded. Open invoice actions to review the payment before correcting the charges.':'Withdraw the invoice first, correct the held shift, then release it for a replacement invoice. The original invoice and this conversation remain in the history.')+'</p><a class="cp-link" href="#/admin/money?section=history&amp;invoice='+e(encodeURIComponent(d.invoice_no))+'">'+(c.can_withdraw===false?'Open invoice & payment actions':'Open withdrawal & correction actions')+'</a></details>';
    }
    return '<section class="cp-query-case" aria-labelledby="cpQueryHeading"><div class="cp-heading"><h2 id="cpQueryHeading" tabindex="-1">'+(office?'Review invoice query':'Question & responses')+'</h2>'+tag({tone:c.state==='needs-reply'&&office?'action':c.state==='resolved'?'success':'waiting',icon:c.state==='resolved'?'✓':c.state==='needs-reply'&&office?'→':'◷',label:c.state==='needs-reply'?'Waiting for an answer':c.state==='awaiting-participant'?'Waiting for participant review':c.state==='withdrawn'?'Invoice withdrawn':'Query resolved'})+'</div>'+(bookings.length>1?'<p class="cp-small">Shifts on this invoice: '+bookings.map(b=>e(date(b.date)+' at '+b.start+' · '+b.hours+' hours'+(b.approval_state==='queried'?' · question open':''))).join('; ')+'.</p>':'')+notes+actions+'</section>';
  }
  function invoiceView(d,publicView=false,draft=null){
  ...
  async function mount(root,opts={}){
    ...
    root.onsubmit=ev=>{const f=ev.target.closest('[data-cp-form]');if(!f||!context(s))return;ev.preventDefault();transact(s,ev.submitter||f.querySelector('button'),async()=>{
      if(['office-reply','request-worker'].includes(f.dataset.cpForm)){
        if(s.publicView||!api()?.me?.admin||!s.data.can_manage_query||!s.data.query_case)throw Error('Office access is required to answer this query.');
        const worker=f.dataset.cpForm==='request-worker',confirmed=!!f.querySelector('[name=confirm]')?.checked,reply=f.querySelector('[name=reply]')?.value||'';
        const key=f.dataset.requestKey||requestKey();if(!worker)s.queryDraft={reply,key};
        if(!confirmed)throw Error(worker?'Confirm that you want to request a worker response.':'Confirm that you have checked your answer before sending it.');
        if(!worker&&(reply.trim().length<10||reply.trim().split(/\s+/).length<3))throw Error('Write a clear answer in at least one sentence.');
        const saved=await call('/admin/invoices/'+encodeURIComponent(s.data.invoice_no)+'/query/'+(worker?'request-worker':'reply'),{method:'POST',body:{fingerprint:s.data.query_case.fingerprint,confirm:true,...(!worker?{reply,idempotency_key:key}:{})}});
        if(!context(s))return;s.queryDraft=null;await reload(s);if(!context(s))return;
        result(s,saved.notice==='missing-recipient'?'Reply saved, but no email could be queued because there is no active reviewer email address. Check the people directory; payment stays paused.':worker?(saved.duplicate?'The worker notification is already queued. Payment stays paused while the query is open.':'Worker notification queued. Payment stays paused while the query is open.'):'Reply saved and notification queued. Waiting for the participant to review; payment stays paused.',saved.notice==='missing-recipient');refreshAlerts(true);return;
      }
      ...
      if(query&&note.trim().length<10)throw Error('Describe your question in at least 10 characters.');
  ...
  function paint(s){if(context(s)){s.root.innerHTML=invoiceView(s.data,s.publicView,s.queryDraft);if(!s.focusedQuery&&q().get('query')==='1'){s.root.querySelector('#cpQueryHeading')?.focus?.();s.focusedQuery=true;}}}
  ...
  async function transact(s,b,fn){if(s.busy)return;s.busy=true;if(b)b.disabled=true;try{await fn();}catch(err){if(context(s)){try{await reload(s);}catch{}result(s,err.message+(s.data?.can_manage_query?' The latest invoice status is shown below.':' Your current shift approval is kept.'),true);}}finally{s.busy=false;if(b?.isConnected)b.disabled=false;}}
  ...
  function officePage(d,view={}){
    ...
    const source=tab==='receipts'?d.transfers||[]:tab==='exceptions'?d.exceptions||[]:tab==='setup'?[]:d.invoices||[];
    const rows=source.filter(row=>{
      ...
      const fields=tab==='invoices'?[row.invoice_no,person(row),row.payer_email]:tab==='receipts'?[person(row),row.reference,row.provider_transaction_id,row.provider,row.invoice_no,row.id]:[row.label,row.message,row.note,row.detail,row.reason,row.kind,row.invoice_no,row.provider];
  ...
  function invoiceRows(rows){return rows.map(i=>`<tr><td><a href="${e(invoiceHref(i.invoice_no))}">${e(i.invoice_no)}</a><br>${e(person(i))}<br><small>${e(i.payer_email||'')}</small><br><a class="cp-row-action" href="#/admin/money?section=history&amp;invoice=${e(encodeURIComponent(i.invoice_no))}">Invoice actions</a>${i.status!=='withdrawn'&&(i.review_state==='queried'||i.status==='under-review')?'<br><a class="cp-row-action cp-query-action" href="'+e(invoiceHref(i.invoice_no)+'&query=1')+'">'+(i.review_state==='queried'?'Review query':'View query & response')+' →</a>':''}</td><td>${tag(state({...i,can_manage_query:true}))}${i.delivery_status?'<br><small>Email: '+e(i.delivery_status)+'</small>':''}</td><td>${money(i.paid)}</td><td>${money(amount(i))}</td><td class="cp-due-date">${date(due(i))}</td><td>${Number(i.paid)>0?e(settlement(i)):'No payment received'}</td></tr>`).join('');}
  ...
  function setupView(d){
    const stripe=d.providers?.stripe||{};
    return `<p>Invoice payments use the secure payment link. Payers choose card or, when enabled, PayTo through Stripe.</p><article class="cp-provider cp-payment-connection"><h2>Card & PayTo</h2>${tag({tone:stripe.configured?'waiting':'action',icon:stripe.configured?'◷':'→',label:stripe.configured?'Stripe settings present':'Stripe not configured'})}<p>PayTo ${stripe.payto_enabled?'enabled':'not enabled'}</p><p>Environment: ${e(stripe.environment||'Not selected')}</p><p class="cp-small">A saved configuration does not confirm a successful payment or deposit. Confirm payment from Stripe’s result and verify deposits using provider and bank records.</p></article><p>Each invoice payment updates automatically after Stripe confirms it. PayTo here is authorised for the selected invoice; it does not automatically charge future shifts.</p>`;
  }
  function officeView(d,view={}){
    ...
    return `<div class="cp-office" data-cp-office-tab="${tab}"><div class="cp-heading"><div><p class="cp-eyebrow">${tab==='setup'?'Settings':'Money'}</p><h1>${officeTabs[tab]}</h1>${summary?'<p>'+summary+'</p>':''}</div><button type="button" data-cp-office-refresh>Refresh</button></div>${fallback}<p role="status" aria-live="polite" id="cpOfficeStatus"></p><section aria-label="${officeTabs[tab]}">${tab==='setup'?setupView(d):officeToolbar(d,v)+'<div data-cp-office-results tabindex="-1">'+officeResults(d,v)+'</div>'}${tab==='exceptions'?'<p><button type="button" data-cp-retry>Retry pending payment jobs</button></p><p class="cp-small">Retrying queues outstanding jobs. Their confirmed result appears after processing.</p>':''}</section>${['invoices','receipts'].includes(tab)?'<p class="cp-small">Includes all records needing action and up to 250 recent closed records. Payment recorded does not by itself confirm a deposit into your business bank account.</p>':''}${tab==='invoices'?'<p class="cp-small">Customer payment does not confirm that wages have been transferred. <a href="#/journey?panel=payroll">Open worker pay</a>.</p>':''}</div>`;
  }
  ...
  async function renderOffice(message=''){
    ...
    try{
      const d=await call('/admin/payments/dashboard');if(!officeCurrent(s))return;
      s.data=d;root.innerHTML=officeView(s.data,s.view);decorateOffice(s);
  ...
  function bindOffice(root,s){
    ...
    root.onsubmit=async ev=>{
      const f=ev.target.closest('[data-cp-office-form]');if(!f)return;ev.preventDefault();if(!officeCurrent(s)||s.busy||s.tab!=='receipts'||f.dataset?.cpOfficeForm!=='allocate')return;
      ...
      try{
        ...
        await call('/admin/payments/transfers/'+encodeURIComponent(f.dataset.transfer)+'/allocate',{method:'POST',body:{invoice_no:value('invoice_no'),amount:Number(value('amount')),confirm:true,idempotency_key:f.dataset.requestKey||(f.dataset.requestKey=requestKey())}});
        const message='Payment allocation saved. Invoice balances have been refreshed.';
