...
- **Shift notes** — the worker cannot mark a shift completed without writing what happened, so there is no note to chase later: no note, no completion, no invoice line, no claim. Notes are **append-only** — a correction is added underneath with its own date, never an edit, because a progress note that can be quietly rewritten afterwards is not evidence. The participant and their support coordinator can read every note about them. Each note also asks the worker *"were you asked to do anything outside what you're booked for?"* — a Yes emails the office straight away and opens a row in the dashboard that can't be closed until somebody records what was done about it (the case in point being **0104 high-intensity supports**, which DMHC is not registered for). One click downloads a participant's whole note history as a CSV for audit sampling.
...
- **Documents & credentials — the Australian catalogue** — the worker Bookings page carries a full onboarding system built around a structured document catalogue (`GET /api/doc-catalog`): identity documents with the standard **100-point ID check** (passport/birth certificate/citizenship certificate = 70-point primaries; driver licence 40; Medicare 25 — a live tally shows e.g. "110 / 100 pts · primary ✓"), right-to-work evidence, checks & clearances (NDIS Worker Screening, WWCC with state guidance, National Police Check), training certificates (NDIS Worker Orientation Module with its free training link, infection control, First Aid/CPR, medication, manual handling) and qualifications/resume (Cert III/IV autocomplete + free-text). Workers **type into a smart search** ("drivers license", "cert 3", "vevo" all match via aliases) and the right fields appear — number label adapts (Passport number / WWCC number), expiry shows only where it applies and is enforced where required. A 7-item **onboarding meter** (photo, screening, 100 points, right to work, orientation, first aid, resume) shows exactly what's missing. All dates display **DD/MM/YYYY**. Admins see each worker's scorecard (ID points, right to work, key training) in the dashboard. Privacy: identity documents and resumes **never** appear on public profiles — only checks, training and qualifications, label + month-level expiry only.
- **The automatic checker** — expiry dates across every document are watched twice a day: email warnings to worker + admin at 30 and 7 days out, and **automatic hiding of any worker whose screening lapses** (with emails both ways). Approval of a new worker is **blocked without a current screening on file** unless the admin ticks an explicit override. Admins record their one-click "verified in the NDIS Worker Screening Database" per document (the NWSD has no public API, so that check is a human step — the system records who and when). Files live in a `bookit-docs` folder next to the database (covered by server snapshots; excluded from git).
...
- **Reviews** — after a completed shift the participant gets a star widget on their Bookings page (1–5 stars + an optional sentence). Reviews appear on the worker's public profile with the reviewer's first name, and live averages replace the demo numbers everywhere. Workers are emailed when a review lands; admins can hide any review from the dashboard.
- **Card payments (Stripe)** — set `STRIPE_SECRET_KEY` and self-managed invoices get a hosted "Pay by card" button; set `STRIPE_WEBHOOK_SECRET` (webhook → `https://bookit.life/api/stripe/webhook`, event `checkout.session.completed`) and paid invoices mark their shifts **paid automatically** the moment the card goes through (signature-verified; the webhook passes the preview gate). No keys = dormant, bank-transfer only.
- **SIL rosters** — the admin dashboard holds shared-living houses, each with a weekly repeating roster (day, start, hours, service, worker, participant, sleepover). One click generates the week's real bookings (pre-accepted, tagged with the house, never duplicated) — completion, invoicing and PACE claiming then flow like any other shift on the 0115 shared-living items.
- **Launch switch** — the admin dashboard's Launch section shows the preview-lock and demo-data status. One button (type LAUNCH to confirm) permanently removes all demo accounts and everything they touched; the login screen stops advertising demo accounts by itself. Then delete `SITE_PASSWORD` from the server env, add `SEED_DEMO=off`, restart — live.
...
| `STRIPE_SECRET_KEY` | (unset) | `sk_live_…` from dashboard.stripe.com → Developers → API keys. Set = self-managed invoices get a pay-by-card link |
| `STRIPE_WEBHOOK_SECRET` | (unset) | `whsec_…` from the Stripe dashboard webhook (endpoint `/api/stripe/webhook`, event `checkout.session.completed`). Set = card payments mark shifts paid automatically |
| `SEED_DEMO` | (unset) | `off` stops a fresh database from seeding demo accounts — set it at launch |
...
- **Worker vetting is in** ✓ — new workers stay hidden until you approve them from `#/admin`, and approval is blocked without a current NDIS Worker Screening on file unless you tick an explicit override. Documents are uploaded and stored in the platform (`bookit-docs`), with expiry dates watched automatically.
- **Shift notes are in** ✓ — every completed shift carries a note, notes can't be edited or deleted, and each participant's file downloads as a CSV. What is *not* automated is the follow-up: an out-of-scope flag rings a bell, but somebody at the office still has to make the call and write down what they said.
...
SQLite tables: `users`, `worker_profiles`, `conversations`, `messages`, `bookings`, `shift_notes`, `contact_messages`. Inspect it any time with `sqlite3 bookit.db` or a GUI like DB Browser for SQLite. Contact-form submissions: `SELECT * FROM contact_messages ORDER BY id DESC;`
