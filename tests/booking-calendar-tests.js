...
function ui(role='worker'){
 const nodes=new Map(),calls=[],timers=new Map(),events={},intervals=[];let timer=0,data=sample(role),count=2,response;
 ...
 const ctx={esc,URLSearchParams,Date,encodeURIComponent,SVC_NAME:{'daily-tasks':'Daily support'},document:{hidden:false,getElementById:get,addEventListener(type,fn){events[type]=fn;}},location:{hash:'#/bookings?view=calendar'},API:{online:true,me:{id:1,role},actingFor:null,async call(url,opts){calls.push({url,opts});if(response)return response(url,opts);if(url==='/me/booking-alerts')return {count,next_date:'2026-09-12'};return structuredClone(data);}},setTimeout(fn,ms){const id=++timer;timers.set(id,{fn,ms});return id;},clearTimeout(id){timers.delete(id);},setInterval(fn,ms){intervals.push({fn,ms});},addEventListener(type,fn){events[type]=fn;}};ctx.window=ctx;vm.createContext(ctx);vm.runInContext(source,ctx);
 return {ctx,get,calls,timers,events,intervals,setData(v){data=v;},setCount(v){count=v;},respond(fn){response=fn;},render:()=>ctx.CareBookingCalendar.render(),action:(a,d)=>get('bookingsList').onclick(event(a,d))};
}
...
await test('Calendar displays server-time dates, overnight carry-over and exact existing booking controls',async()=>{
 const v=ui();await v.render();const out=v.get('bookingsList').innerHTML;assert.match(out,/Times shown in Australia\/Sydney/);assert.match(out,/Continues from 11 Sept? · ends 09:00/);assert.match(out,/bookings\?view=list&amp;booking=3&amp;date=2026-09-12/);assert.match(out,/aria-current="date"/);assert.equal((out.match(/tabindex="0"/g)||[]).length,1);assert.ok(!out.includes('Cancelled example'));assert.ok(!out.includes('name="note"'));
});
