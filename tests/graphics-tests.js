...
t('original route map remains with additive journey, recovery and payment routes',hash((routes||'').replace("\n  '/journey': 'page-journey',",'').replace("\n  '/login': 'page-login',",'').replace("\n  '/not-found': 'page-not-found',",'').replace("\n  '/invoice': 'page-invoice',",'').replace("\n  '/payment-tracking': 'page-payment-tracking',",'').replace("\n  '/email-change': 'page-email-change',",''))===baseline.route_map_sha256);
const ids=[...new Set([...html.matchAll(/id="(page-[\w-]+)"/g)].map(m=>m[1]))].sort();
t('all page containers remain',JSON.stringify(ids.filter(id=>!['page-journey','page-login','page-not-found','page-invoice','page-payment-tracking','page-email-change'].includes(id)))===JSON.stringify(baseline.page_ids));
...
const ver=require('../lib/version');t('additive account email history schema is identified as 88404',ver.SCHEMA_VERSION===88404);
