...
async function main() {
  ...
  /* ---- v86.9.0: inactive night care, meet-and-greets, the feed, referrals, KPIs, fees, suburb pages ---- */
  {
    ...
    t('the PDF is a real tax invoice: address, item names, times, due date, balance, payment link', pdf.status === 200 && pdfText.startsWith('%PDF') && pdfText.includes('16 Crystal Crescent') && pdfText.includes('Access Community Social and Rec Activ') && (pdfText.includes('11:00\\226') || pdfText.includes('11:00')), pdf.status);
    t('… naming the NDIS item and the balance due', pdfText.includes('04_104_0125_6_1') && pdfText.includes('Balance due') && pdfText.includes('secure invoice link') && pdfText.includes('/Subtype /Link') && !/Bank transfer|BSB:|Account number:/i.test(pdfText));
