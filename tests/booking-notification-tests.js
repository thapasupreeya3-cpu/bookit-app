...
function fixture(){
  ...
  const c={db,emailOn:()=>state.enabled,mailPrefs:id=>prefs[id]||{},baseUrl:()=> 'https://care.example.test',sendMailDirect:async(...args)=>{if(state.failure)throw Error('Synthetic temporary transport outage');sent.push(args);return {accepted:true};},
    prettyDate:d=>d,serviceLabels:{'personal-care':'Personal care'},escHtml:s=>String(s).replace(/[&<>"']/g,x=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[x])),bookingStart:b=>new Date(b.date+'T'+b.start+':00Z'),blockedPair:()=>state.blocked,
...
(async()=>{
  await test('New requests acknowledge the participant and scoped helper and prompt the worker',async()=>{
    ...
    for(const r of rows){assert.equal(r.urgent,1);const a=JSON.parse(r.payload);assert.equal(a[8].booking_id,11);assert.doesNotMatch(r.payload,/PRIVATE ARRIVAL NOTES/);assert.match(a[5],/#\/bookings\?booking=11/);}
