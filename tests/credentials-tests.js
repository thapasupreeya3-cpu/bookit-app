...
const supplied=()=>['passport-au','driver-licence','ndis-screening','ndis-orientation','first-aid','cpr','resume'].map((doc_type,i)=>({id:i+1,doc_type,file_name:doc_type+'.pdf',has_file:true,status:'valid',review:'submitted'}));
await test('Received essentials have a pending summary; only verified usable evidence produces the verified summary',async()=>{
 const v=ui(supplied());await v.render();assert.match(v.wrap.innerHTML,/<div class="cred-overview is-pending">/);assert.match(v.wrap.innerHTML,/<strong>Essential files received<\/strong>/);assert.match(v.wrap.innerHTML,/7 essential requirements are awaiting office review/);assert.ok(!v.wrap.innerHTML.includes('<strong>Essential documents verified</strong>'));
 const docs=supplied().map(d=>({...d,review:'approved',verified_at:'2026-09-10T10:30:00Z'}));const complete=ui(docs);await complete.render();assert.match(complete.wrap.innerHTML,/<div class="cred-overview is-verified">/);assert.match(complete.wrap.innerHTML,/<strong>Essential documents verified<\/strong>/);assert.match(complete.wrap.innerHTML,/All 7 essential requirements are verified/);assert.match(complete.wrap.innerHTML,/Verified 2026-09-10/);
 docs.find(d=>d.doc_type==='cpr').status='expired';const expired=ui(docs);await expired.render();assert.match(expired.wrap.innerHTML,/<div class="cred-overview is-missing">/);assert.match(expired.wrap.innerHTML,/1 essential item needs a file/);assert.ok(!expired.wrap.innerHTML.includes('<strong>Essential documents verified</strong>'));
});
await test('Mixed evidence keeps precise pending counts and old rejected records cannot erase current verification',async()=>{
 const docs=supplied().map(d=>({...d,review:'approved',verified_at:'2026-09-10'}));docs.find(d=>d.doc_type==='cpr').verified_at='';const v=ui(docs);await v.render();assert.match(v.wrap.innerHTML,/1 essential requirement is awaiting office review/);
 docs.find(d=>d.doc_type==='cpr').verified_at='2026-09-10';docs.push({id:100,doc_type:'cpr',has_file:true,status:'valid',review_state:'rejected',review_note:'Please upload a clearer copy'});const history=ui(docs);await history.render();assert.match(history.wrap.innerHTML,/<div class="cred-overview is-verified">/);assert.match(history.wrap.innerHTML,/Please upload a clearer copy/);assert.match(history.wrap.innerHTML,/Replace requested/);
});
await test('View document status focuses and scrolls to the checklist without navigating or changing data',async()=>{
 const v=ui();await v.render();const count=v.calls.length;await v.action('status','');const title=v.wrap.querySelector('#credChecklistTitle');assert.equal(title.focused,true);assert.equal(title.scrolled,true);assert.equal(v.ctx.location.hash,'#/account/credentials');assert.equal(v.calls.length,count);
 const stale=ui();await stale.render();stale.ctx.location.hash='#/bookings';await stale.action('status','');assert.ok(!stale.wrap.querySelector('#credChecklistTitle').scrolled);
});
await test('Shared identity evidence and platform completions explain where their supporting records are',async()=>{
 const docs=supplied().map(d=>({...d,verified_at:'2026-09-10'}));Object.assign(docs.find(d=>d.doc_type==='ndis-orientation'),{has_file:false,evidence_origin:'platform-module'});const v=ui(docs);await v.render();assert.match(v.wrap.innerHTML,/Covered by Australian Passport in your identity documents/);assert.match(v.wrap.innerHTML,/No separate upload needed for this completion/);assert.match(v.wrap.innerHTML,/How verification works/);assert.match(v.wrap.innerHTML,/Uploading a file does not verify it/);assert.ok(!v.wrap.innerHTML.includes('data-cred-action="replace" data-value="4"'));await v.action('identity','');assert.equal(v.wrap.querySelector('[data-cred-requirement="identity"] h4').scrolled,true);
});
