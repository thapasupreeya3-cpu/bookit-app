...
function checkAssets(dir){for(const entry of fs.readdirSync(path.join(ROOT,dir),{withFileTypes:true})){const name=dir+'/'+entry.name;if(entry.isDirectory())checkAssets(name);else if(name.endsWith('.js'))check(name,fs.readFileSync(path.join(ROOT,name),'utf8'));}}
checkAssets('public/assets');
