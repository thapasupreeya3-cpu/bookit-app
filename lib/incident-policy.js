...
module.exports={VERSION,text,businessDays,deadlines,mount(c){const{db,now,setting}=c;
 ...
 const holidays=()=>{try{return JSON.parse(setting('incident_holidays','[]')).filter(T.validDate);}catch{return [];}};
 ...
 for(const i of db.prepare("SELECT * FROM incidents WHERE COALESCE(rule_version,'')='' AND reportable=1").all()){
  const d=deadlines({category:i.category,created:i.created,occurred_at:i.created},holidays());
