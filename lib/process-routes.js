...
module.exports=function mount(c,w){
 ...
 add('GET',/^\/api\/admin\/journey-queue$/,(req,res,m,u)=>{
   ...
   const rows=db.prepare("SELECT t.*,u.name AS person,u.role AS person_role FROM journey_tasks t JOIN users u ON u.id=t.user_id WHERE t.state<>'completed' AND (?='' OR u.name LIKE ? OR t.label LIKE ?) ORDER BY COALESCE(t.due_at,'9999'),t.ready_at LIMIT 100 OFFSET ?").all(search,'%'+search+'%','%'+search+'%',offset);
   ...
   const staff=db.prepare("SELECT id,name FROM users WHERE is_admin=1 AND COALESCE(closed_at,'')=''").all();json(res,200,{rows,total,offset,staff,today:c.ymd(),as_of:now()});
 });
