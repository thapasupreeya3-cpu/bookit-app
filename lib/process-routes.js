...
module.exports=function mount(c,w){
 ...
 add('GET',/^\/api\/journey$/,(req,res,m,u)=>{
   ...
   let tasks=ids?db.prepare("SELECT * FROM journey_tasks WHERE user_id=? AND state<>'completed'").all(ids[0]):db.prepare("SELECT t.*,u.name AS person FROM journey_tasks t JOIN users u ON u.id=t.user_id WHERE t.state<>'completed' ORDER BY COALESCE(t.due_at,'9999'),t.ready_at LIMIT 1000").all();
   ...
   json(res,200,{tasks,next,scopes,subject:ids?{id:uid,name:db.prepare('SELECT name FROM users WHERE id=?').get(uid)?.name}:null,role:admin(u)?'office':u.role,preferences:w.preferences(u.id),summary:{do_now:tasks.filter(t=>t.owner_kind==='person').length,office:tasks.filter(t=>t.owner_kind==='office').length}});
 });
 ...
 add('GET',/^\/api\/admin\/journey-queue$/,(req,res,m,u)=>{
   ...
   const rows=db.prepare("SELECT t.*,u.name AS person FROM journey_tasks t JOIN users u ON u.id=t.user_id WHERE t.state<>'completed' AND (?='' OR u.name LIKE ? OR t.label LIKE ?) ORDER BY COALESCE(t.due_at,'9999'),t.ready_at LIMIT 100 OFFSET ?").all(search,'%'+search+'%','%'+search+'%',offset);
   const total=db.prepare("SELECT count(*) n FROM journey_tasks t JOIN users u ON u.id=t.user_id WHERE t.state<>'completed' AND (?='' OR u.name LIKE ? OR t.label LIKE ?)").get(search,'%'+search+'%','%'+search+'%').n;
   const staff=db.prepare("SELECT id,name FROM users WHERE is_admin=1 AND COALESCE(closed_at,'')=''").all();json(res,200,{rows,total,offset,staff});
 });
 ...
 add('GET',/^\/api\/journey\/intake$/,(req,res,m,u)=>{
   ...
   json(res,200,{revision:r?.revision||0,data:{name:account.name,phone:account.phone,suburb:account.suburb,ec_name:plan?.ec_name||'',ec_phone:plan?.ec_phone||'',ec_relationship:plan?.ec_relationship||'',...parse(r?.data)},updated_at:r?.updated_at||'',support_plan_changed:!!(r&&plan&&r.updated_at>plan.confirmed_at)});
 });
 add('PUT',/^\/api\/journey\/intake$/,(req,res,m,u,b)=>{
   ...
   w.event(u.id,'intake','saved',`intake:${p.id}:${(cur?.revision||0)+1}`);json(res,200,{ok:true,revision:(cur?.revision||0)+1,plan_update_offered:!!c.confirmedPlan(p.id)});
 });
 ...
 add('POST',/^\/api\/journey\/series\/(\d+)\/accept$/,(req,res,m,u,b)=>{
   ...
   db.exec('BEGIN IMMEDIATE');try{
     ...
     const p=c.sessionUser(visits[0].participant_id);c.notify(p.id,'bookings',p.email,'Your repeating visits are confirmed','Visits confirmed',`<p>${visits.length} reviewed visits have been accepted. Open your diary to see each date.</p>`,'Open visits',c.baseUrl(req)+'/#/journey');db.exec('COMMIT');
 ...
 add('GET',/^\/api\/admin\/deliveries$/,(req,res)=>{json(res,200,{email_enabled:c.emailOn(),rows:db.prepare("SELECT id,kind,recipient,status,attempts,next_at,created_at,sent_at,error,urgent FROM delivery_outbox ORDER BY CASE status WHEN 'failed' THEN 0 WHEN 'retry' THEN 1 ELSE 2 END,id DESC LIMIT 200").all()});});
 ...
 add('POST',/^\/api\/admin\/journey-settings$/,(req,res,m,u,b)=>{
   ...
   if(b.private_billing_ready&&(!['0','10'].includes(String(b.private_gst_percent))||clean(b.private_billing_evidence,1000).length<20))return fail(res,'Record the private billing review and tax treatment.');
   ...
   if(b.private_billing_ready!==undefined){if(b.private_billing_ready&&(!['0','10'].includes(String(b.private_gst_percent))||clean(b.private_billing_evidence,1000).length<20))return fail(res,'Record the private billing review and tax treatment.');c.setSetting('private_billing_ready',b.private_billing_ready?'on':'off');c.setSetting('private_gst_percent',String(b.private_gst_percent||0));c.setSetting('private_billing_evidence',clean(b.private_billing_evidence,1000));}
