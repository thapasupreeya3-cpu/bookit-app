...
module.exports=function mount(c,w){
 ...
 add('GET',/^\/api\/admin\/journey-queue$/,(req,res,m,u)=>{
   if(!admin(u))return fail(res,'Office access required.',403);
   ...
   const page=require('./task-queue').queue(db,{view:q.get('view'),search,offset,group:q.get('group'),person:clean(q.get('person'),80),today:c.ymd(),as_of:now()});
 ...
 add('GET',/^\/api\/journey\/bookings\/(\d+)$/,(req,res,m,u)=>{
   ...
   const notes=db.prepare("SELECT n.id,n.body,n.kind,n.created,n.addendum,ua.name AS author_name,ua.role AS author_role FROM shift_notes n LEFT JOIN users ua ON ua.id=COALESCE(n.author_id,n.worker_id) WHERE n.booking_id=? ORDER BY n.id").all(b.id).map(n=>({...n,author_label:n.kind==='question'&&u.role==='worker'&&n.author_role==='coordinator'?'their support coordinator':n.author_name||(n.kind==='office-reply'?'Office':n.kind==='question'?'Participant':'Worker')}));
   json(res,200,{booking:c.bookingPriceView?c.bookingPriceView(b):b,scope:c.scopeState?.(b),other_name:party?.name||'',notes,brief:permitted?c.workerBrief(b.participant_id):null,plan_id:permitted?plan?.id:null,plan_version:permitted?plan?.version:null,ack:permitted&&u.role==='worker'?c.planAck(b.participant_id,u.id):null,series:b.series_id?db.prepare('SELECT id,date,start,hours,status FROM bookings WHERE series_id=? AND worker_id=? ORDER BY date').all(b.series_id,b.worker_id):[]});
 });
