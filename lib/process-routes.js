...
module.exports=function mount(c,w){
 ...
 add('GET',/^\/api\/journey\/intake$/,(req,res,m,u)=>{
   ...
   json(res,200,{revision:r?.revision||0,data:{ec_name:plan?.ec_name||'',ec_phone:plan?.ec_phone||'',ec_relationship:plan?.ec_relationship||'',...parse(r?.data),name:account.name,phone:account.phone,suburb:account.suburb},updated_at:r?.updated_at||'',support_plan_changed:!!(r&&plan&&r.updated_at>plan.confirmed_at)});
 });
