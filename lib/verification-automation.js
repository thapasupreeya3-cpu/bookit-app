...
module.exports = function verificationAutomation(c, w, review) {
  ...
  const sources = [['users','id'],['worker_profiles','user_id'],['worker_docs','worker_id'],['participant_docs','participant_id'],
    ['support_plans','participant_id'],['interview_slots','worker_id'],['bookings','worker_id'],['recruitment_checks','worker_id'],['module_completions','worker_id'],['verification_cases','user_id'],['verification_followups','user_id'],['verification_plan_snapshots','user_id']];
  ...
  function refreshQueue(role = '') {
    let rebuilt = 0;
    const timeKey='worker-readiness-v3:'+c.ymd()+':'+Math.floor(Date.now()/300000); // Reclassify existing office placeholders on upgrade, and bound elapsed-time staleness.
    ...
    for (const row of rows) {
      ...
      const data = {id:s.id,name:s.name,role:s.role,email:s.email,suburb:s.suburb,email_confirmed:s.email_confirmed,demo:s.demo,
        state:s.state,state_label:s.state_label,ready:s.ready,next:s.next,office_action_count:s.office_action_count,waiting_reason:s.waiting_reason,pending_count:s.pending_count,document_count:s.document_count,
  ...
  function assistance(p) {
    ...
    return {enabled:c.setting('ai_document_processing','off')==='on'&&c.AI.live()&&c.AI.on('credential-extract')&&c.AI.onshore()&&c.AI.approved()&&!!c.AI.endpoint(),suggestions};
  }
