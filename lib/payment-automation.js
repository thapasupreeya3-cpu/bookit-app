...
module.exports = function createPayments(c, w) {
  ...
  // Receiving details remain in the ledger for historical reconciliation only.
  // Stale Zai configuration or BANK_DETAILS must never reopen a payment route.
  function bankFor() { return null; }
  ...
  function summary(inv,req,user,publicAccess=false,queryDetail=true) {
    const s=state(inv.invoice_no), allowedPay=canPay(req,user,inv,publicAccess);
    const queryAccess=!publicAccess&&(!!user?.admin||canReview(req,user,inv));
    const queryCase=queryAccess?(queryDetail?w.invoiceQueries?.view(inv.invoice_no):w.invoiceQueries?.summary(inv.invoice_no)):undefined;
    ...
    return {invoice_no:inv.invoice_no,funding:inv.funding,withdrawn:!!inv.withdrawn,total:inv.total,paid:inv.paid,balance:inv.balance,
      ...
      query_note:queryAccess?s.rows.find(b=>b.query_note)?.query_note||'':'',
      query_case:queryCase,query_state:queryCase?.state,can_manage_query:!publicAccess&&!!user?.admin,
      ...
      bank:null,payment_policy:'invoice-link-only',
      ...
      settlement_status:'not_confirmed',pause_reason:queryAccess?s.access?.pause_reason||'':'',
  ...
  async function checkout(inv,method) {
    if(!c.stripeEnabled())throw Object.assign(Error('Online payment is temporarily unavailable. Return to this invoice link to try again, or contact the office for help.'),{status:409});
  ...
  function recordStripe(eventData) {
    ...
    if(fail){
      ...
      if(getInvoice(no)?.balance>0){
        event(no,e.id,'payment-failed','Payment did not complete');
        // Expiring or cancelling a link is normal checkout housekeeping, not a
        // failed collection. Related session/intent webhooks describe the same
        // attempt and must not each email the payer.
        if(['checkout.session.async_payment_failed','payment_intent.payment_failed'].includes(e.type)&&!['paid','cancelled'].includes(a?.state))queuePaymentFailure(no,a?.id||link?.checkout_id||s.id,e.id);
      }
  ...
  function hasLegacyZaiAccount(environment) {
    return !!db.prepare('SELECT 1 FROM payment_accounts WHERE environment=? LIMIT 1').get(environment);
  }
  function hasLegacyZaiTransaction(environment,transactionId) {
    if(db.prepare("SELECT 1 FROM payment_transfers WHERE provider='zai' AND environment=? AND provider_transaction_id=?").get(environment,transactionId))return true;
    return db.prepare("SELECT payload FROM payment_provider_jobs WHERE provider='zai'").all().some(job=>{
      try{const payload=JSON.parse(job.payload);return payload.environment===environment&&payload.transactionId===transactionId;}catch{return false;}
    });
  }
  async function zaiWebhook(req,res,raw) {
    try {
      const config=zai.getStatus(),legacyAccount=hasLegacyZaiAccount(config.environment);
      // Keep signed confirmations for accounts issued before retirement. An
      // environment variable alone is not evidence of an existing connection.
      if(!legacyAccount){
        let transactionId;try{transactionId=JSON.parse(raw)?.transactions?.id;}catch{}
        if(!transactionId||!hasLegacyZaiTransaction(config.environment,transactionId))return json(res,410,{error:'Bank transfers have been retired. Please use the payment link on your invoice.'});
      }
      if(!config.configured)return json(res,503,{error:'Historical bank payment confirmation needs its existing provider connection restored.'});
      ...
      if(checked.kind==='transaction')enqueueProvider('zai','zai:'+config.environment+':'+checked.eventKey,{...checked,environment:config.environment});
  ...
  function queuePaymentFailure(no,key,providerEvent) {
    const inv=getInvoice(no),meta={payment_failure:true,payment_failure_attempt:key};
    if(!inv||paymentDeliveryInvalid(no,meta)||!inv.payer_email)return;
    c.sendMail(inv.payer_email,'Payment needs attention — '+no,'Payment not completed',
      '<p>Your payment for invoice '+c.escHtml(no)+' was not completed. Your shift approval is saved. Open the invoice link to try again using an available payment method.</p>',
      'View invoice',invoiceUrl(no),undefined,[],{kind:'invoice',transactional:true,event_key:'payment-failed:'+no+':'+key,invoice_no:no,payment_reminder:true,...meta,payment_provider_event:providerEvent});
  }
  ...
  function paymentDeliveryInvalid(no,meta={}) {
    const s=state(no);
    if(!s.inv||s.inv.withdrawn||s.paused||!s.approved||s.inv.balance<=0)return true;
    // A reminder queued before the payer started a bank payment must not ask
    // them to pay twice while provider confirmation is still outstanding.
    if(db.prepare("SELECT 1 FROM payment_attempts WHERE invoice_no=? AND state='processing'").get(no))return true;
    const sourceKey=String(meta.invoice_link_source_event_key||meta.event_key||'');
    if(meta.payment_failure||sourceKey.startsWith('payment-failed:')){
      let attempt=db.prepare('SELECT * FROM payment_attempts WHERE id=? OR checkout_id=?').get(meta.payment_failure_attempt||'',meta.payment_failure_attempt||'');
      if(!meta.payment_failure){
        // Retained pre-update failures identify their provider event in the
        // outbox key. Recover its attempt before applying the same freshness
        // checks, including messages already prepared for a transport retry.
        const old=db.prepare('SELECT * FROM finance_provider_events WHERE event_id=?').get(sourceKey.slice('payment-failed:'.length));
        if(!old||old.invoice_no!==no||!['checkout.session.async_payment_failed','payment_intent.payment_failed'].includes(old.event_type))return true;
        attempt=db.prepare('SELECT * FROM payment_attempts WHERE checkout_id=? OR payment_intent=?').get(old.object_id||'',old.payment_intent||old.object_id||'');
        if(!attempt&&db.prepare("SELECT 1 FROM payment_attempts WHERE invoice_no=? AND (created_at>=? OR state IN ('creating','retry','ready','processing'))").get(no,old.created_at))return true;
      }
      if(attempt){
        if(attempt.invoice_no!==no||!['failed','expired'].includes(attempt.state))return true;
        if(db.prepare("SELECT 1 FROM payment_attempts WHERE invoice_no=? AND id<>? AND (created_at>? OR state IN ('creating','retry','ready','processing'))").get(no,attempt.id,attempt.created_at))return true;
      }
    }
    return false;
  }
  w.paymentDeliveryInvalid=paymentDeliveryInvalid;
  async function tick() {
    ...
    ticking=(async()=>{
      ...
      for(const j of db.prepare("SELECT * FROM payment_provider_jobs WHERE state<>'complete' AND next_at<=? ORDER BY created_at LIMIT 50").all(now())){
        try{
          ...
          if(j.provider==='stripe')recordStripe(payload);
          else if(j.provider==='zai') {
            // A persisted job itself is evidence of a historical notification;
            // never drop it merely because new bank transfers are now retired.
            const config=zai.getStatus();
            if(payload.environment!==config.environment)throw Error('This notification belongs to another payment environment.');
            if(!config.configured)throw Error('Historical bank payment confirmation needs its existing provider connection restored.');
            ...
            // The invoice link is the only current payment route. A late legacy
            // deposit stays visible for office reconciliation rather than paying
            // a newly issued invoice from a reference on an old bank transfer.
          ...
          }else throw Error('Unknown payment provider notification.');
          db.prepare("UPDATE payment_provider_jobs SET state='complete',error='',attempts=attempts+1 WHERE event_key=?").run(j.event_key);
  ...
  c.route('POST',/^\/api\/admin\/payments\/zai\/accounts$/,protect((req,res,m,user)=>{
    if(!user?.admin)return json(res,403,{error:'Admin only.'});
    return json(res,410,{error:'Bank-transfer receiving accounts have been retired. Payers must use the secure payment link on their invoice.',payment_policy:'invoice-link-only'});
  }));
  ...
  function dashboard(req,user) {
    ...
    for(const row of db.prepare('SELECT invoice_no FROM invoice_snapshots ORDER BY created_at DESC,invoice_no DESC').all()){
      ...
      invoices.push({...summary(inv,req,user,false,false),participant:inv.participant.name,participant_id:inv.participant.id,payer_email:inv.payer_email,delivery_status:db.prepare('SELECT status FROM delivery_outbox WHERE event_key=?').get('invoice:'+inv.invoice_no)?.status||'unknown'});
    }
    ...
    return {providers:{stripe:{configured:c.stripeEnabled(),payto_enabled:c.paytoEnabled(),environment:c.stripeEnvironment()}},payment_policy:'invoice-link-only',invoices,transfers,
  ...
  w.paymentTasks=uid=>db.prepare('SELECT DISTINCT invoice_no FROM bookings WHERE participant_id=? AND COALESCE(invoice_no,\'\')<>\'\'').all(uid).map(r=>{
    ...
    const status=invoiceStatus(inv,s),review=!s.approved&&s.review!=='queried';
