...
function validateBrowserRequest(req, configuredAppUrl, options = {}) {
  ...
  const exempt = options.exemptPaths || ["/api/stripe/webhook", "/stripe/webhook", "/api/webhooks/stripe", "/api/zai/webhook"];
...
function strongSecret(value) {
  ...
  return s.length>=32 && new Set(s).size>=10 && !/^(change[-_ ]?me|example|your[-_ ]|replace[-_ ])/i.test(s) && !/^(.{1,16})\1+$/.test(s);
}
function selectedSecret(env,rootDir) {
  const key=['SESSION_SECRET','BOOKIT_SESSION_SECRET','BOOKIT_SECRET','SECRET'].find(k=>Object.hasOwn(env,k));
  if(key)return String(env[key]).trim();
  const file=env.SECRET_FILE||path.join(rootDir,'.secret');
  try { return fs.readFileSync(file,'utf8').trim(); } catch(e) {
    if(env.NODE_ENV==='production'||e.code!=='ENOENT')throw Error('The selected session secret cannot be read');
    const value=crypto.randomBytes(32).toString('hex');fs.writeFileSync(file,value,{mode:0o600,flag:'wx'});return value;
  }
}
function validateProductionEnvironment(options = {}) {
  ...
  const selected = ['SESSION_SECRET','BOOKIT_SESSION_SECRET','BOOKIT_SECRET','SECRET'].find(k => Object.hasOwn(env,k));
  let secret = '';
  try { secret = String(selected ? env[selected] : fs.readFileSync(env.SECRET_FILE || path.join(rootDir,'.secret'),'utf8')).trim(); }
  catch { errors.push("The selected session secret cannot be read"); }
  if (!strongSecret(secret)) errors.push("The selected session secret must contain at least 32 characters and must not be a placeholder or repeated value; generate 32 random bytes");
