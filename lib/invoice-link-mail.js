...
module.exports=function invoiceLinkMail(c){
  return async function prepare(row,args){
    ...
    next[1]=previouslyAttempted?'Updated payment instructions — '+no:invoiceDemand?'Invoice '+no+' — The Care Web':'Payment update — '+no;
    next[2]=previouslyAttempted?'Updated payment instructions':invoiceDemand?'Your invoice is ready':'Check your invoice payment';
