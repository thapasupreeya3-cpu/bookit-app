...
module.exports=function(c,w,h){
 ...
 function receipt(invoice,user,b){
   ...
   (w.paymentTransactionActive ? fn=>fn() : tx)(()=>{
 ...
 function prepare(from,to,actor,expectedPayDate){
   ...
   if(expectedPayDate!==undefined&&expectedPayDate!==''&&expectedPayDate!==null&&!T.validDate(expectedPayDate))throw new Error('Use a real expected pay date, or leave it blank.');
   ...
   const result=tx(()=>{const id=Number(db.prepare('INSERT INTO payroll_batches(from_date,to_date,created_at,created_by) VALUES(?,?,?,?)').run(from,to,now(),actor).lastInsertRowid);
   ...
   if(expectedPayDate!==undefined)w.payrollNotices?.recordExpectedDate(result.id,expectedPayDate);
   w.payrollNotices?.prepared(result.id);
   return result;
 }
 ...
 add('POST',/^\/api\/admin\/payroll-batches$/,(req,res,m,u,b)=>{try{json(res,200,prepare(clean(b.from,10),clean(b.to,10),u.id,b.expected_pay_date));}catch(e){fail(res,e.message);}});
 ...
 add('POST',/^\/api\/admin\/payroll-batches\/(\d+)\/acknowledge$/,(req,res,m,u,b)=>{
   ...
   tx(()=>{db.prepare("UPDATE payroll_batches SET status='acknowledged',external_ref=?,acknowledged_at=?,acknowledged_by=? WHERE id=?").run(clean(b.external_ref,200),now(),u.id,p.id);for(const l of p.lines.filter(l=>l.status==='included')){db.prepare('UPDATE payroll_lines SET external_ref=? WHERE id=?').run(clean(b.external_ref,200),l.id);if(l.source_key.startsWith('referral:'))db.prepare('UPDATE referrals SET paid_at=?,paid_by=? WHERE id=? AND paid_at IS NULL').run(now(),u.name,Number(l.source_key.split(':')[1]));}w.event(u.id,'payroll','payment-recorded','payroll:'+p.id+':paid');});w.payrollNotices?.acknowledged(p.id);json(res,200,{ok:true});
 });
 add('GET',/^\/api\/journey\/payroll$/,(req,res,m,u)=>{if(u.role!=='worker')return fail(res,'Workers only.',403);if(w.payrollNotices)return json(res,200,w.payrollNotices.workerData(u.id));json(res,200,{lines:db.prepare('SELECT l.id,l.source_key,l.amount,l.status,l.resolution,b.status AS batch_status,b.from_date,b.to_date,b.acknowledged_at FROM payroll_lines l JOIN payroll_batches b ON b.id=l.batch_id WHERE l.worker_id=? ORDER BY l.id DESC LIMIT 200').all(u.id)});});
 ...
 w.scheduledPayroll=()=>{
   if(c.setting('payroll_drafts_enabled','off')!=='on')return {disabled:true,reason:'Office confirms the payroll cutover before scheduled preparation.'};
   const cutover=c.setting('payroll_cutover_date','');if(!T.validDate(cutover))throw Error('A payroll cutover date is required.');
   const yesterday=new Date();yesterday.setDate(yesterday.getDate()-1);const end=c.ymd(yesterday),batches=[];
   // Scan the complete authorised cutover range in bounded periods. prepare()
   // retains its source-key deduplication and every existing payroll review.
   for(let from=cutover;from<=end;){
     const boundary=new Date(from+'T12:00:00Z');boundary.setUTCDate(boundary.getUTCDate()+89);
     const to=boundary.toISOString().slice(0,10)<end?boundary.toISOString().slice(0,10):end;
     const result=prepare(from,to,0);if(result.id)batches.push(result.id);
     const next=new Date(to+'T12:00:00Z');next.setUTCDate(next.getUTCDate()+1);from=next.toISOString().slice(0,10);
   }
   return {ok:true,empty:batches.length===0,batches,created:batches.length,from:cutover,to:end};
 };
};
