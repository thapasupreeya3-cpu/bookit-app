...
module.exports=function(c,w,h){
 ...
 function receipt(invoice,user,b){
   const ref=clean(b.reference,150),note=clean(b.note||b.how,1000),amount=Number(b.amount);
   if(b.confirm!==true||ref.length<5||note.length<10||!Number.isFinite(amount)||amount<=0)throw Error('Record the payment reference, actual amount and evidence, then confirm your review.');
   const old=db.prepare('SELECT * FROM invoice_payment_evidence WHERE reference=?').get(ref);
   if(old){if(old.invoice_no!==invoice.invoice_no||Math.abs(old.amount)!==money(amount))throw Error('This payment reference is already used for a different receipt.');return {ok:true,duplicate:true,state:old.state};}
   const reverse=b.paid===false,expected=reverse?invoice.paid:invoice.balance;
   if(expected<=0)throw Error(reverse?'This invoice is not marked paid.':'This invoice is already settled.');
   const match=money(amount)===money(expected),state=match?(reverse?'reversed':'recorded'):'exception';
   tx(()=>{db.prepare('INSERT INTO invoice_payment_evidence(invoice_no,reference,amount,recorded_by,recorded_at,note,state) VALUES(?,?,?,?,?,?,?)').run(invoice.invoice_no,ref,reverse?-money(amount):money(amount),user.id,now(),note,state);
     if(match)db.prepare('UPDATE bookings SET claim_status=?,paid_at=? WHERE invoice_no=?').run(reverse?'claimed':'paid',reverse?null:now(),invoice.invoice_no);
     w.event(user.id,'invoice',state,'receipt:'+ref);});
   return {ok:true,state,matched:match,message:match?'Payment evidence recorded.':'The amount does not match the invoice balance. The receipt is held for reconciliation; the invoice was not marked paid.'};
 }
 w.recordPayment=receipt;
 w.recordStripePayment=event=>{
   const s=event.data?.object||{},no=s.metadata?.invoice_no,inv=no?c.invoiceFor(no):null;if(!inv)return false;
   const ref='stripe:'+String(event.id||'');if(!event.id||db.prepare('SELECT 1 FROM invoice_payment_evidence WHERE reference=?').get(ref))return false;
   const session=db.prepare('SELECT stripe_session FROM bookings WHERE invoice_no=? LIMIT 1').get(no)?.stripe_session;
   if(event.type==='checkout.session.completed'&&s.payment_status==='unpaid'&&s.id===session)return false;
   const matches=s.payment_status==='paid'&&s.currency==='aud'&&Number.isInteger(s.amount_total)&&s.amount_total===Math.round(inv.total*100)&&!!session&&s.id===session;
   tx(()=>{db.prepare('INSERT INTO invoice_payment_evidence(invoice_no,reference,amount,recorded_by,recorded_at,note,state) VALUES(?,?,?,?,?,?,?)').run(no,ref,Number(s.amount_total||0)/100,0,now(),matches?'Signed Stripe event matched the stored checkout session, AUD amount and paid status.':'Signed Stripe event did not match the stored session, amount, currency or paid status. Review with the payment provider.',matches?'recorded':'exception');if(matches)db.prepare("UPDATE bookings SET claim_status='paid',paid_at=? WHERE invoice_no=? AND claim_status<>'paid'").run(now(),no);});return matches;
 };
 ...
 function batch(id){const b=db.prepare('SELECT * FROM payroll_batches WHERE id=?').get(Number(id));return b?{...b,lines:db.prepare('SELECT * FROM payroll_lines WHERE batch_id=? ORDER BY id').all(b.id).map(l=>({...l,data:parse(l.data)}))}:null;}
 ...
 add('POST',/^\/api\/admin\/payroll-lines\/(\d+)\/review$/,(req,res,m,u,b)=>{
   ...
   db.prepare('UPDATE payroll_lines SET status=?,resolution=? WHERE id=?').run(b.status,clean(b.reason,1000),l.id);w.event(u.id,'payroll','line-reviewed','pay-line:'+l.id+':'+now());json(res,200,{ok:true});
 });
 ...
 add('GET',/^\/api\/admin\/payroll-batches\/(\d+)\.csv$/,(req,res,m,u)=>{
   const p=batch(m[1]);if(!p||!['approved','exported','acknowledged'].includes(p.status))return fail(res,'Review and approve the batch before export.',409);if(p.status==='approved'&&stale(p).length)return fail(res,'A source changed after approval. Reopen and review the batch.',409);
   const q=c.csvCell,rows=[['Batch ID','Line ID','Source','Worker ID','Worker','Date','Type','Amount incl. super','Review reason'].map(q).join(',')];for(const l of p.lines.filter(l=>l.status==='included'))rows.push([p.id,l.id,l.source_key,l.worker_id,l.data.worker,l.data.date,l.data.type,l.amount.toFixed(2),l.resolution].map(q).join(','));
