...
function mayReadCurrent(ctx, workerId, participantId) {
  ...
  if (!p || p.closed_at || (!p.visible && !p.self_paused) || !ctx.platformEligible(workerId,p.email) || ctx.participantClosed(participantId)) return false;
  const active = ctx.activeBookings(workerId,participantId);
  if (active.some(b => (p.visible || b.status==='accepted') && +localStart(b)+Number(b.hours)*36e5 > Date.now() && !['finding','office','uncovered','failed','allied','referred','stood-down'].includes(b.cover_state))) return true;
