...
module.exports=function(c,w){
 ...
 async function drain(){if(running)return running;running=(async()=>{
   ...
   // Historic receipt evidence alone cannot invalidate a newly created checkout
   // for the remaining balance. The payment module knows the current attempt's
   // amount/fingerprint; legacy sessions retain the conservative evidence rule.
   const links=db.prepare("SELECT DISTINCT b.invoice_no,b.stripe_session,b.claim_status,EXISTS(SELECT 1 FROM invoice_payment_evidence e WHERE e.invoice_no=b.invoice_no AND e.state IN ('recorded','reversed','opening')) AS has_payment FROM bookings b WHERE COALESCE(b.pay_url,'')<>''").all();
   for(const link of links){
     const retire=link.claim_status==='paid'||(w.paymentLinkNeedsRetirement?w.paymentLinkNeedsRetirement(link.invoice_no,link.stripe_session):!!link.has_payment);
     if(!retire)continue;
     queueCheckout(link.invoice_no,link.stripe_session);
     db.prepare("UPDATE bookings SET pay_url='' WHERE invoice_no=? AND stripe_session IS ?").run(link.invoice_no,link.stripe_session);
   }
   for(const r of db.prepare("SELECT * FROM checkout_cleanup WHERE state IN ('queued','retry') AND next_at<=? ORDER BY next_at LIMIT 25").all(now())){
     try{
       ...
       if(s.status==='expired')db.prepare("UPDATE checkout_cleanup SET state='closed',error='',attempts=attempts+1,updated_at=? WHERE session_id=?").run(now(),r.session_id);
         ...
         const known=db.prepare('SELECT 1 FROM invoice_payment_evidence WHERE reference=?').get('stripe:checkout:'+r.session_id)||w.paymentSessionRecorded?.(r.session_id);
