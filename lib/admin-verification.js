...
module.exports = function mountVerification(c, workflow) {
  ...
  function summary(p) {
    ...
    const activationEligible=worker?platform.ok&&!missing.length&&!!profile.photo&&!profile.self_paused&&!!p.verified:!missing.length;
    const activation=worker?tasks.find(t=>t.key==='activation'):null;
    const ready=worker&&!profile.visible&&!profile.self_paused?!!activation?.actionable:activationEligible;
    ...
    // Eligibility blockers describe the whole application. Only checks with
    // something for the office to act on belong in its review workload.
    const officeTasks=tasks.filter(t=>t.yours===false&&t.actionable!==false&&(t.key!=='activation'||ready));
    const officeReviewTasks=officeTasks.filter(t=>t.key!=='activation');
    let state=safety?'blocked':worker&&profile.visible&&platform.ok?'active':ready?'ready':pending.length||planReview||officeReviewTasks.length?'review':'waiting';
    // New evidence remains visible even for an already active worker.
    if(!safety&&(pending.length||planReview||officeReviewTasks.length))state='review';
    ...
    const labels={blocked:'Needs attention',active:'Active',ready:worker?'Ready to activate':'Ready to book',review:'Office review',waiting:worker?'Waiting on worker':'Waiting on participant'};
    // Pending files are individual review actions, not a second count of the
    // corresponding setup verify-* placeholder. Activation remains a decision.
    const officeCount=pending.length+officeTasks.filter(t=>!worker||!t.key.startsWith('verify-')||!pending.some(d=>d.doc_type===t.key.slice(7))).length+Number(planReview)+Number(followupReview);
    const office_action_count=Math.max(officeCount,Number(safety),Number(worker&&ready&&!profile.visible));
    const waitingForDocuments=worker&&!docs.some(d=>d.has_file||d.accepted_at)&&state==='waiting';
    const waiting_reason=state==='waiting'?(waitingForDocuments?'The worker needs to upload their documents and complete their profile. No office action is needed yet.':worker?'The worker still has information or setup items to complete. Office checks will appear when the required information is available.':'The participant still has information or setup items to complete. No office action is needed yet.') : '';
    const first=pending[0];
    const next=first?`Review ${first.type_label}`:planReview?'Review the confirmed support plan':safety?'Review the safety restriction':followupReview?'Follow up on the outstanding checklist':worker&&ready&&!profile.visible?'Make the activation decision':officeReviewTasks[0]?.label||(state==='waiting'?(waitingForDocuments?'Waiting for worker documents':labels[state]):labels[state]);
    const received=pending.map(d=>d.uploaded_at||d.created_at||d.doc_date).filter(Boolean).sort()[0]||null;
    return {id:p.id,name:p.name,role:p.role,email:p.email,suburb:p.suburb,email_confirmed:!!p.verified,demo:c.isDemoWorker(p.email),state,state_label:labels[state],ready,activation_eligible:activationEligible,next,office_action_count,waiting_reason,pending_count:pending.length,document_count:docs.length,received_at:received,case:file,tasks,platform,profile,missing,plan,planState,documents:docs,plan_review:planReview};
  }
  function detail(p) {
  ...
  route('POST', /^\/api\/admin\/verification\/(\d+)\/action$/, async(req,res,m,u,b={},ip)=>{
    ...
    if(['verify-document','return-document'].includes(action)){
      ...
      const s=summary(p);if(!s.activation_eligible||s.profile.visible||v.confirm!==true)return fail(res,'Complete the outstanding eligibility checks and confirm the activation decision.',409);
