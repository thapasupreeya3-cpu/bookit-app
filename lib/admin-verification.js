...
module.exports = function mountVerification(c, workflow) {
  ...
  route('GET', /^\/api\/document-viewer$/, (req,res,m,u)=>{
    ...
    if(!['worker','participant'].includes(q.get('scope'))||!/^\d+$/.test(id||'')||!Number.isSafeInteger(Number(id))||Number(id)<1)return fail(res,'Choose a worker or participant document.');
    res.writeHead(200,{'Content-Type':'text/html; charset=utf-8','X-Frame-Options':'SAMEORIGIN','Content-Security-Policy':"default-src 'none'; script-src 'self' 'wasm-unsafe-eval'; worker-src 'self'; connect-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self'; frame-ancestors 'self'; object-src 'none'; base-uri 'none'; form-action 'none'",'Cache-Control':'private, no-store','X-Content-Type-Options':'nosniff','Referrer-Policy':'no-referrer'});
