...
module.exports = function mountVerification(c, workflow) {
  ...
  function documentRows(p) { return rawDocs(p).map(d=>{
    const out=(p.role==='worker'?c.docOut:c.pdocOut)(d),cat=(p.role==='worker'?c.docMap[d.doc_type]:c.pdocMap[d.form_key])||{};
    return {...out,source_url:d.file_path?`/api/${p.role==='worker'?'documents':'participant-documents'}/${d.id}/file`:null,purpose:[cat.points?`${cat.points} identity points`:null,cat.primary?'Primary identity':null,cat.rtw?'Work rights':null,cat.requires].filter(Boolean).join(' · ')};
  }); }
  function token(p) {
    ...
    const fields={last_action:db.prepare('SELECT MAX(id) AS id FROM verification_activity WHERE user_id=?').get(p.id).id,person:db.prepare('SELECT * FROM users WHERE id=?').get(p.id),profile,docs:rawDocs(p),plans,
      ...
      requests:c.openRequests(p.role,p.id),privateReady:c.setting('private_billing_ready','off'),date:c.ymd()};
  ...
  function summary(p) {
    ...
    const pending=active.filter(d=>!d.verified_at&&!d.accepted_at),file=db.prepare('SELECT v.*,u.name AS owner_name FROM verification_cases v LEFT JOIN users u ON u.id=v.owner_id WHERE v.user_id=?').get(p.id)||{user_id:p.id,owner_id:null,due_date:'',note:''};
    ...
    const planReview=!!plan&&(planState.awaiting_first_review||planState.changed_since_review);
    ...
    const next=first?`Review ${first.type_label}`:planReview?'Review the confirmed support plan':safety?'Review the safety restriction':worker&&ready&&!profile.visible?'Make the activation decision':tasks.find(t=>t.key!=='activation')?.label||labels[state];
  ...
  function detail(p) {
    const s=summary(p),worker=p.role==='worker';
    return {...s,person:p,review_token:token(p),staff:staff(),methods:worker?methods:c.pdocMethods,
  ...
  route('GET', /^\/api\/admin\/verification$/, (req,res,m,u)=>{
    ...
    const q=new URL(req.url,'http://local').searchParams,role=q.get('role')||'worker',filter=q.get('status')||'all',search=clean(q.get('q'),100).toLowerCase(),owner=q.get('owner')||'all',offset=Number(q.get('offset')||0);
    if(!['worker','participant'].includes(role)||!['all','review','waiting','ready','active','blocked'].includes(filter)||!['all','mine','unassigned'].includes(owner)||!Number.isInteger(offset)||offset<0)return fail(res,'Choose valid queue filters.');
    let rows=db.prepare("SELECT id,name,role,email,phone,suburb,verified,created,plan,closed_at FROM users WHERE role=? AND COALESCE(is_admin,0)=0 AND COALESCE(closed_at,'')='' ORDER BY name,id").all(role)
      .filter(p=>q.get('examples')==='1'||!c.isDemoWorker(p.email)).filter(p=>!search||[p.name,p.email,p.suburb].some(x=>String(x||'').toLowerCase().includes(search))).map(summary);
    if(owner==='mine')rows=rows.filter(x=>x.case.owner_id===u.id);if(owner==='unassigned')rows=rows.filter(x=>!x.case.owner_id);
    const counts={all:rows.length,review:0,waiting:0,ready:0,active:0,blocked:0};rows.forEach(x=>counts[x.state]++);
    if(filter!=='all')rows=rows.filter(x=>x.state===filter);
    const order={blocked:0,review:1,ready:2,waiting:3,active:4};rows.sort((a,b)=>order[a.state]-order[b.state]||String(a.case.due_date||'9999').localeCompare(b.case.due_date||'9999')||String(a.received_at||'9999').localeCompare(b.received_at||'9999')||a.name.localeCompare(b.name));
    json(res,200,{role,counts,total:rows.length,offset,limit:30,rows:rows.slice(offset,offset+30).map(({tasks,platform,profile,missing,plan,planState,documents,...x})=>({...x,case:{owner_id:x.case.owner_id,owner_name:x.case.owner_name,due_date:x.case.due_date}}))});
  });
