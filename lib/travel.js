...
async function live(from, to) {
  ...
  const trafficAware=!/^(off|0|false|no)$/i.test(process.env.GOOGLE_MAPS_TRAFFIC || '');
  const ck = `${trafficAware}|${String(from).toLowerCase()}|${String(to).toLowerCase()}`;
  const hit = LIVE_CACHE.get(ck);
  if (hit && hit.until > Date.now()) return hit.value ? describe(hit.value) : null;
  ...
  try {
    const r = await fetch(url, { method: 'POST', signal: ac.signal,
      ...
      body: JSON.stringify({ origins: [{ waypoint: { address: `${from}, Australia` } }], destinations: [{ waypoint: { address: `${to}, Australia` } }],
        travelMode: 'DRIVE', routingPreference: trafficAware ? 'TRAFFIC_AWARE' : 'TRAFFIC_UNAWARE', regionCode: 'AU', languageCode: 'en-AU' }) });
    ...
    if (r.ok && el && Number.isFinite(secs) && Number.isFinite(el.distanceMeters)) {
      ...
      value = { known: true, km, minutes, source: 'google', traffic, checked_at: new Date().toISOString(), from, to };
    }
  } catch { value = null; }
  clearTimeout(timer);
  /* v88.1.4 (audit F09): a traffic-aware figure is kept for six hours, a
     traffic-unaware one for a day; either way the answer says when it was
     checked, so nobody reads a morning figure as tonight's traffic. */
  LIVE_CACHE.set(ck, { value, until: Date.now() + (value ? (value.traffic ? 6 : 24) : 1) * 3600e3 });
  return value ? describe(value) : null;
}
/* the words for a Google figure, worked out when it is shown rather than when it was fetched */
function describe(v) {
  const ageMin = Math.max(0, Math.round((Date.now() - new Date(v.checked_at).getTime()) / 60000));
  const when = ageMin < 2 ? 'checked just now' : ageMin < 60 ? `checked ${ageMin} min ago` : `checked ${fmtMinutes(ageMin)} ago`;
  return { ...v, age_minutes: ageMin,
    text: `about ${fmtMinutes(v.minutes)} by car (${v.km} km) between ${v.from} and ${v.to}${v.traffic ? ', with typical traffic' : ''} — a planning estimate, ${when}` };
}
