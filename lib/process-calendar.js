...
module.exports=function(c,w,h){
 ...
 function render(uid,pid,feedId){
   ...
   const rows=db.prepare(`SELECT * FROM bookings WHERE ${col}=? AND status IN ('accepted','completed','cancelled') AND date>=? ORDER BY date,id`).all(subject,c.ymd(new Date(Date.now()-90*864e5)));
   for(const prev of db.prepare('SELECT * FROM calendar_feed_events WHERE feed_id=?').all(feedId))if(!rows.some(b=>b.id===prev.booking_id)){const b=JSON.parse(prev.data);if(c.bookingEnd(b)>=new Date(Date.now()-90*864e5))b.status='cancelled';rows.push(b);}
   for(const b of rows){
 ...
 add('GET',/^\/api\/calendar\/([a-f0-9]{64})\.ics$/,(req,res,m)=>{
   ...
   res.writeHead(200,{'Content-Type':'text/calendar; charset=utf-8','Referrer-Policy':'no-referrer','Content-Disposition':'inline; filename="careweb-visits.ics"'});res.end(body);
 });
