...
function areaMatches(area, place) {
  ...
  if (!a || !p) return false;
  if (a === p) return true;
  // Match an explicitly recorded whole postcode in either direction. Names
  // without postcodes are never converted into codes or matched by substring.
  if (/^\d{4}$/.test(a)) return p.split(' ').includes(a);
  if (/^\d{4}$/.test(p)) return a.split(' ').includes(p);
  // Structured addresses add state/postcode suffixes to the same suburb.
  // Compare whole suburb names and reject conflicting explicit states/postcodes.
  const parts=value=>{let name=value,postcode='',state='';const pc=/\s+(\d{4})$/.exec(name);if(pc){postcode=pc[1];name=name.slice(0,pc.index);}const st=/\s+(nsw|vic|qld|act|sa|wa|tas|nt)$/.exec(name);if(st){state=st[1];name=name.slice(0,st.index);}return {name,state,postcode};};
  const left=parts(a),right=parts(p);
  if(left.name===right.name&&(!left.state||!right.state||left.state===right.state)&&(!left.postcode||!right.postcode||left.postcode===right.postcode))return true;
  return false;
}
...
function availability(profile, b, participantPlace) {
  ...
  /* v88.1.3 (audit F02): the area check comes last, after leave, time windows and
     weekdays, so an out-of-area confirmation only ever waives the area. */
