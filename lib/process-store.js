...
module.exports=function createStore(c){
 ...
 // Retain safe message labels after the body and attachments are purged.
 for(const [name,type] of [['subject',"TEXT NOT NULL DEFAULT ''"],['heading',"TEXT NOT NULL DEFAULT ''"],['event_kind',"TEXT NOT NULL DEFAULT ''"],['booking_id','INTEGER']])if(!db.prepare('PRAGMA table_info(delivery_outbox)').all().some(x=>x.name===name))db.exec(`ALTER TABLE delivery_outbox ADD COLUMN ${name} ${type}`);
 const labelUpdate=db.prepare('UPDATE delivery_outbox SET subject=?,heading=?,event_kind=?,booking_id=? WHERE id=?');
 for(const row of db.prepare("SELECT id,payload FROM delivery_outbox WHERE subject='' AND payload<>'[]'").all()){
   const args=parse(row.payload,[]),meta=args[8]||{};
   if(Array.isArray(args)&&args[1])labelUpdate.run(String(args[1]).slice(0,300),String(args[2]||'').slice(0,300),String(meta.event_kind||'').slice(0,80),Number.isSafeInteger(meta.booking_id)?meta.booking_id:null,row.id);
 }
 // Previous releases stopped timesheet and payroll mail after five transport
 // failures. Resume only retained unsent bodies; live task and access checks
 // below still cancel notices which are no longer relevant.
 db.prepare("UPDATE delivery_outbox SET status='retry',next_at=?,lease_until=NULL WHERE kind IN ('timesheets','payroll') AND status='failed' AND payload<>'[]'").run(now());
 for(const row of db.prepare("SELECT id,kind,payload FROM delivery_outbox WHERE kind IN ('timesheets','payroll') AND status IN ('queued','retry') AND urgent=0").all()){
   const args=parse(row.payload,[]),meta=args[8]||{};
   if(Array.isArray(args)&&(meta.requires_approval===true||meta.booking_shift_event||/^shift-/.test(meta.event_kind||'')||row.kind==='payroll'&&meta.payroll_update)){
     args[8]={...meta,transactional:true};
     db.prepare('UPDATE delivery_outbox SET urgent=1,next_at=?,payload=? WHERE id=?').run(now(),JSON.stringify(args),row.id);
   }
 }
 ...
 function enqueueMail(args){
   ...
   // The logical outbox key must also reach the provider. Messages without an
   // explicit caller key still need the same transport identity after a retry.
   payload.push({...meta,event_key:key});
   ...
   db.prepare("UPDATE delivery_outbox SET subject=?,heading=?,event_kind=?,booking_id=? WHERE event_key=? AND subject=''").run(String(subject||'').slice(0,300),String(heading||'').slice(0,300),String(meta.event_kind||'').slice(0,80),Number.isSafeInteger(meta.booking_id)?meta.booking_id:null,key);
 ...
 function suppress(row){
   ...
   if(meta.payment_reminder&&api.paymentDeliveryInvalid?.(meta.invoice_no,meta))return 'Payment settled, withdrawn or under review';
   if(meta.payroll_update&&deliveryHooks.payroll){const why=deliveryHooks.payroll(row,args);if(why)return why;}
   if(meta.invoice_query_notice&&deliveryHooks.invoiceQuery){const why=deliveryHooks.invoiceQuery(row,args);if(why)return why;}
   if(meta.account_email_change&&deliveryHooks.accountEmail){const why=deliveryHooks.accountEmail(row,args);if(why)return why;}
   if(meta.location_revision!==undefined&&deliveryHooks.location){const why=deliveryHooks.location(row,args);if(why)return why;}
   if(meta.verification_followup&&deliveryHooks.verification){const why=deliveryHooks.verification(row,args);if(why)return why;}
   if(meta.booking_lifecycle&&deliveryHooks.booking){const why=deliveryHooks.booking(row,args);if(why)return why;}
   if(meta.event_kind==='booking-request'&&!meta.booking_lifecycle){const booking=db.prepare('SELECT status,worker_id FROM bookings WHERE id=?').get(meta.booking_id);if(!booking||booking.status!=='requested'||booking.worker_id!==row.user_id)return 'booking request resolved or reassigned';}
   ...
   const u=db.prepare('SELECT role,email,closed_at FROM users WHERE id=?').get(row.user_id);
   if(!u||u.closed_at)return 'access changed';
   if(accessStamp(row.user_id)!==row.access_stamp){
     // Booking suppression above checks this participant's current permission.
     // An unrelated helper link must not discard a still-authorised receipt.
     // Identity changes remain disqualifying, even if a hook permits the visit.
     const expectedRole={participant:'participant',helper:'coordinator',worker:'worker'}[meta.booking_recipient_role];
     const scopedBooking=meta.booking_lifecycle&&typeof deliveryHooks.booking==='function'&&expectedRole===u.role&&String(u.email||'').trim().toLowerCase()===row.recipient;
     if(!scopedBooking)return 'access changed';
   }
   if(row.kind!=='invoice'&&row.kind&&c.mailPrefs(row.user_id)[row.kind]===false)return 'notification preference';
   return '';
 }
 function drain(){
   // Both the scheduled job and immediate invoicing wake this same outbox.
   // Share a running pass so a second wake cannot race its retry decisions.
   if(deliveryRun)return deliveryRun;
   deliveryRun=Promise.resolve().then(drainPass).finally(()=>{deliveryRun=null;});
   return deliveryRun;
 }
 async function drainPass(){
   ...
   for(const row of rows){
     ...
     group=group.filter(x=>db.prepare("UPDATE delivery_outbox SET status='sending',lease_until=?,attempts=attempts+1 WHERE id=? AND status IN ('queued','retry') AND next_at<=? AND attempts=?").run(new Date(Date.now()+120000).toISOString(),x.id,now(),x.attempts).changes);
     if(!group.length)continue;
     try{let args;
       for(const x of group){
         let prepared=parse(x.payload,[]);prepared[7]=(prepared[7]||[]).map(({base64,...a})=>({...a,buffer:Buffer.from(base64,'base64')}));
         // Older queued messages may predate persisted provider keys. Keep an
         // explicit transport revision (for example corrected invoice mail).
         prepared[8]={...prepared[8],event_key:prepared[8]?.event_key||x.event_key};
         if(deliveryHooks.prepare){
           const updated=await deliveryHooks.prepare(x,prepared);
           if(updated&&updated!==prepared){
             prepared=updated;
             const stored=[...prepared];stored[7]=(prepared[7]||[]).map(a=>({...a,buffer:undefined,base64:Buffer.from(a.buffer).toString('base64')}));
             db.prepare("UPDATE delivery_outbox SET payload=? WHERE id=? AND status='sending'").run(JSON.stringify(stored),x.id);
           }
         }
         x.prepared=prepared;
       }
       // Preparation may await PDF generation. Keep withdrawal and access
       // suppression effective if those permissions changed while it ran.
       group=group.filter(x=>{
         if(db.prepare('SELECT status FROM delivery_outbox WHERE id=?').get(x.id)?.status!=='sending')return false;
         const why=suppress(x);if(!why)return true;
         db.prepare("UPDATE delivery_outbox SET status='cancelled',error=?,payload='[]',lease_until=NULL WHERE id=? AND status='sending'").run(why,x.id);return false;
       });
       if(!group.length)continue;
       args=group[0].prepared;
       if(!row.urgent&&pref.digest&&!args[7].length)args=[row.recipient,'Your Care Web daily updates','Your next actions',`<p>You have ${group.length} routine updates. Open your next actions to see what still needs attention.</p>`,'Open next actions',c.baseUrl({headers:{}})+'/#/journey',undefined,[],{event_key:'daily-digest:'+hash(group.map(x=>x.event_key).sort())}];
       const result=await c.sendMailDirect(...args);
       if(typeof result==='string'&&result.startsWith('skipped-'))throw Error('The email service did not accept this message: '+result);
       for(const x of group)db.prepare("UPDATE delivery_outbox SET status='sent',sent_at=?,error='',lease_until=NULL,payload='[]' WHERE id=?").run(now(),x.id);sent+=group.length;
     }catch(e){for(const x of group){const attempts=x.attempts+1,durable=['invoice','bookings','timesheets','payroll'].includes(x.kind);const changed=db.prepare("UPDATE delivery_outbox SET status=?,next_at=?,error=?,lease_until=NULL WHERE id=? AND status='sending'").run(attempts>=5&&!durable?'failed':'retry',new Date(Date.now()+(durable?Math.min(3600000,15000*2**Math.min(attempts,8)):row.urgent?15000:Math.min(3600000,30000*2**attempts))).toISOString(),String(e.message).slice(0,300),x.id);if(changed.changes)failed++;}}
   }
   return {sent,failed};
 }
 ...
 function workerBlockers(uid){
   ...
   const tasks=[];const add=(key,label,yours,where,why='',blocking=true,due=null,actionable=true,waiting_for='')=>tasks.push({key,section:key.startsWith('training')?'training':'credentials',label,yours,where,why,blocking,due,actionable,waiting_for});
   ...
   // Readiness changes the office workload, never the requirements for approval.
   // Keep future decisions in the full checklist without creating a job that
   // cannot yet be carried out. Reviews can still be recorded in any order.
   const checks=Object.fromEntries(db.prepare('SELECT * FROM recruitment_checks WHERE worker_id=?').all(uid).map(x=>[x.check_key,x]));
   const interviewBooked=!!db.prepare("SELECT 1 FROM interview_slots WHERE worker_id=? AND status IN ('booked','finished')").get(uid);
   const recruitmentReady={interview:sum.id_ok||current.some(d=>d.doc_type==='resume')||interviewBooked,references:checks.interview?.status==='complete',employment:checks.interview?.status==='complete'&&checks.references?.status==='complete'};
   const recruitmentWaiting={interview:'Waiting for a resume, identity documents or a booked interview.',references:'Available after the interview review, or when the office starts this check.',employment:'Available after interview and reference reviews, or when the office starts this check.'};
   if(!p.visible&&!p.self_paused)for(const key of ['interview','references','employment'])if(checks[key]?.status!=='complete'){
     const started=!!(checks[key]?.evidence?.trim()||checks[key]?.reviewed_at);
     add('recruit-'+key,'Office review: '+key,false,'#/journey?panel=recruitment','The office records the evidence and outcome.',true,null,recruitmentReady[key]||started,recruitmentWaiting[key]);
   }
   const st=c.platformStatus(uid);
   const established=!!(p.visible||p.self_paused||p.auto_hidden||db.prepare("SELECT 1 FROM bookings WHERE worker_id=? AND status IN ('accepted','completed') LIMIT 1").get(uid));
   const screeningReady=established||current.some(d=>d.doc_type==='ndis-screening')||!!(p.screening_ref?.trim()||p.screening_app_number?.trim()||p.nwsd_linked_at||st.screening?.status_at)||['suspended','revoked','excluded'].includes(st.screening?.status);
   if(st.screening?.status!=='cleared')add('screening-status','Office confirms screening status',false,'#/journey','Confirm the worker’s screening status against the screening unit.',true,null,screeningReady,'Waiting for screening evidence or an application/check number.');
   for(const r of st.registers||[])if(r.result!=='clear'||r.age_days===null||r.age_days>c.banningWindowDays())add('register-'+r.key,'Office check: '+r.short,false,'#/journey','Record the register result and date.',true,null,sum.id_ok||established||!!r.checked_at||r.result==='banned','Waiting for enough identity evidence to check the correct person.');
   if(st.block?.on||p.platform_block)add('safety','Review the safety restriction',false,'#/admin/verification?role=worker&person='+uid,st.block?.reason||p.platform_block_reason||'An office restriction is in place.');
   ...
   if(!p.visible&&!p.self_paused)add('activation','Office activation decision',false,'#/journey',st.blocks.join(' '),true,null,st.ok&&!activationMissing(uid).length&&!tasks.some(t=>t.blocking),'Waiting for the required profile details, evidence and checks to be complete.');
 ...
 function projection(uid){
   ...
   if(u.role==='participant'){
     ...
     for(const b of workerBlockers(uid)){
       if(b.key==='paused'||(!b.yours&&b.actionable===false))continue;
       // Uploaded evidence already has a precise doc-review task below.
       if(!b.yours&&b.key.startsWith('verify-')&&db.prepare("SELECT 1 FROM worker_docs WHERE worker_id=? AND doc_type=? AND COALESCE(file_path,'')<>'' AND COALESCE(verified_at,'')='' AND COALESCE(review_state,'') NOT IN ('rejected','superseded')").get(uid,b.key.slice(7)))continue;
       const recruitment=b.key.startsWith('recruit-');
       add(recruitment?'recruit:'+b.key.slice(8):'setup:'+b.key,recruitment?'recruitment':b.key==='safety'?'safeguarding':'setup',b.label,b.yours?'person':'office',b.where,b.why,'',b.due);
     }
   }
   const paymentTasks=api.paymentTasks?.(uid)||[];
   out.push(...paymentTasks);
   out.push(...(api.locationTasks?.(uid)||[]));
   ...
   for(const b of db.prepare(`SELECT * FROM bookings WHERE ${col}=? AND (status IN ('requested','accepted') OR (status='completed' AND approval_state IN ('pending','queried'))) ORDER BY date,id`).all(uid)){
     ...
     if(b.status==='completed'&&((u.role==='participant'&&b.approval_state==='pending')||(u.role==='worker'&&b.approval_state==='queried'))&&!paymentTasks.some(t=>t.kind==='review'&&(t.task_key===`${uid}:review:${b.id}`||(b.invoice_no&&t.destination==='#/invoice?invoice='+encodeURIComponent(b.invoice_no)))))add('review:'+b.id,'review',b.approval_state==='queried'?'Answer a timesheet question':'Review timesheet '+date,'person',`#/journey?panel=shift&booking=${b.id}`,'','bookings',b.approval_state==='pending'?new Date(Date.parse(b.approval_from||b.completed_at||now())+7*864e5).toISOString():null);
 ...
 function syncOffice(){
   ...
   for(const t of [...(api.verificationOfficeTasks?.()||[]),...(api.launchOfficeTasks?.()||[]),...(api.paymentOfficeTasks?.()||[])])add(t.key,t.kind,t.label,t.dest,t.detail,t.due);
