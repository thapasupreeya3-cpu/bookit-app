...
module.exports=function createStore(c){
 ...
 function enqueueMail(args){
   ...
   const urgent=['security','compliance','cover'].includes(meta.kind)||/cover|incident|confirm your email|password|screening|clearance|invitation/i.test(subject);
   ...
   if(!urgent&&p.digest){next.setDate(next.getDate()+1);next.setHours(8,0,0,0);}
 ...
 function suppress(row){
   ...
   if(row.kind&&c.mailPrefs(row.user_id)[row.kind]===false)return 'notification preference';
   return '';
 }
 async function drain(){
   ...
   for(const row of rows){
     ...
     }catch(e){for(const x of group){const attempts=x.attempts+1;db.prepare("UPDATE delivery_outbox SET status=?,next_at=?,error=?,lease_until=NULL WHERE id=?").run(attempts>=5?'failed':'retry',new Date(Date.now()+(row.urgent?15000:Math.min(3600000,30000*2**attempts))).toISOString(),String(e.message).slice(0,300),x.id);failed++;}}
   }
   return {sent,failed};
 }
 ...
 function invoiceSnapshot(no){const r=db.prepare('SELECT data FROM invoice_snapshots WHERE invoice_no=?').get(no);if(!r)return null;const inv=parse(r.data,null);if(!inv)return null;const rows=db.prepare('SELECT id,claim_status,paid_at,pay_url FROM bookings WHERE invoice_no=?').all(no);if(!rows.length)return null;const paid=rows.every(r=>r.claim_status==='paid');return {...inv,paid:paid?inv.total:0,balance:paid?0:inv.total,paid_at:paid?rows[0].paid_at:'',pay_url:rows[0].pay_url||inv.pay_url};}
 ...
 function workerBlockers(uid){
   ...
   const tasks=[];const add=(key,label,yours,where,why='',blocking=true)=>tasks.push({key,section:key.startsWith('training')?'training':'credentials',label,yours,where,why,blocking});
   if(!u.verified)add('verify-email','Confirm your email address',true,'#/account');
   if(!p.photo)add('photo','Add your profile photo',true,'#/account/credentials');
   const docs=db.prepare("SELECT * FROM worker_docs WHERE worker_id=? AND COALESCE(review_state,'')<>'rejected'").all(uid);
   const sum=c.onboardingSummary(uid);
   if(!sum.id_ok)add('identity','Complete your identity documents',true,'#/journey?panel=uploads',`${sum.id_points} of 100 identity points on file`);
   if(!sum.right_to_work)add('right-to-work','Add your work-rights evidence',true,'#/journey?panel=uploads');
   for(const key of ['ndis-screening','ndis-orientation','first-aid','cpr']){
     const list=docs.filter(d=>d.doc_type===key&&(!d.expiry_date||d.expiry_date>=c.ymd()));const good=list.find(d=>d.verified_at);
     if(!list.length)add('upload-'+key,'Add current '+(c.docMap[key]?.label||key),true,'#/journey?panel=uploads&type='+key);
   ...
   for(const m of c.moduleState(uid).modules.filter(m=>m.required&&(!m.done||m.overdue_days>0)))add('training-'+m.key,m.title,true,'#/account/training','Complete or renew this module',m.overdue_days>=14);
 ...
 function projection(uid){
   ...
   if(u.role==='participant'){
     ...
     for(const b of workerBlockers(uid))if(b.key!=='paused')add('setup:'+b.key,'setup',b.label,b.yours?'person':'office',b.where,b.why);
   ...
   for(const d of db.prepare(`SELECT * FROM ${table} WHERE ${idcol}=? AND COALESCE(review_state,'') NOT IN ('rejected','superseded')`).all(uid)){
     ...
     if(d.expiry_date&&d.expiry_date<=c.ymd(new Date(Date.now()+60*864e5))) {
       ...
       if(!replacement)add('renew:'+d.id,'renewal','Renew '+label,'person',u.role==='worker'?'#/journey?panel=uploads&type='+key:'#/account/documents','Expires '+d.expiry_date,'documents',d.expiry_date+'T23:59:59Z');
     }
   }
   for(const r of c.openRequests(u.role,uid))add('request:'+r.id,'document',r.label,'person',u.role==='worker'?'#/journey?panel=uploads&type='+r.doc_key:'#/account/documents',r.note,'documents',r.due_date?r.due_date+'T23:59:59Z':null);
 ...
 return {closePersonal,event,preferences,accessStamp,enqueueMail,drain,storeInvoice,invoiceSnapshot,bookingAllowed,duplicateDocument,approvalRecipients,workerBlockers,activationMissing,sync,syncAll,cleanup,stageTime,renewalNeeded,parse,hash,now};
};
