...
module.exports=function createStore(c){
 ...
 async function drain(){
   ...
   for(const row of rows){
     ...
     }catch(e){for(const x of group){const attempts=x.attempts+1;const changed=db.prepare("UPDATE delivery_outbox SET status=?,next_at=?,error=?,lease_until=NULL WHERE id=? AND status='sending'").run(attempts>=5&&x.kind!=='invoice'?'failed':'retry',new Date(Date.now()+(x.kind==='invoice'?Math.min(3600000,15000*2**Math.min(attempts,8)):row.urgent?15000:Math.min(3600000,30000*2**attempts))).toISOString(),String(e.message).slice(0,300),x.id);if(changed.changes)failed++;}}
   }
   return {sent,failed};
 }
