...
module.exports=function createStore(c){
 ...
 function invoiceSnapshot(no){const r=db.prepare('SELECT data FROM invoice_snapshots WHERE invoice_no=?').get(no);if(!r)return null;const inv=parse(r.data,null);if(!inv)return null;const rows=db.prepare('SELECT id,claim_status,paid_at,pay_url FROM bookings WHERE invoice_no=?').all(no);if(!rows.length)return null;const paid=rows.every(r=>r.claim_status==='paid');const ledger=api.paymentBalance?.(no,inv.total,paid?inv.total:0);return {...inv,paid:ledger?.paid??(paid?inv.total:0),balance:ledger?.balance??(paid?0:inv.total),paid_at:paid?rows[0].paid_at:'',pay_url:rows[0].pay_url||inv.pay_url};}
